...
public class SwaggerConfigurationTests
{
    ...
    public void AddSwaggerDocumentation_DeveIncluirComentariosXml_QuandoArquivoExiste()
    {
        ...
        try
        {
            ...
            // Act
            var possuiFiltroComentarioXml =
                swaggerOptions.Value.OperationFilterDescriptors.Any(item => item.Type.Name.Contains("XmlComments"))
                || swaggerOptions.Value.ParameterFilterDescriptors.Any(item => item.Type.Name.Contains("XmlComments"))
                || swaggerOptions.Value.RequestBodyFilterDescriptors.Any(item => item.Type.Name.Contains("XmlComments"))
                || swaggerOptions.Value.SchemaFilterDescriptors.Any(item => item.Type.Name.Contains("XmlComments"));

            // Assert
            possuiFiltroComentarioXml.ShouldBeTrue();
        }
        finally
        {
