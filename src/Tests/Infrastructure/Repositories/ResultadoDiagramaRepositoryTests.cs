...
public class ResultadoDiagramaRepositoryTests : IDisposable
{
    ...
    public async Task SalvarERecuperar_DevePersistirAnaliseResultado_QuandoAnalisado()
    {
        ...
        recuperado.Status.Valor.ShouldBe(StatusAnaliseEnum.Analisado);
        recuperado.AnaliseResultado.ShouldNotBeNull();
        recuperado.AnaliseResultado.DescricaoAnalise.Valor.ShouldBe("Análise arquitetural gerada para testes");
    ...
    public async Task SalvarERecuperar_DevePersistirErros_QuandoFalhaProcessamento()
    {
        ...
        // Assert
        recuperado.ShouldNotBeNull();
        recuperado.Status.Valor.ShouldBe(StatusAnaliseEnum.Erro);
        recuperado.Erros.ShouldNotBeEmpty();
        recuperado.Erros[0].Mensagem.Valor.ShouldBe("Falha de processamento simulada");
    }
    ...
    public async Task SalvarAsync_DeveAtualizar_QuandoResultadoJaExistir()
    {
        ...
        recuperado.Status.Valor.ShouldBe(StatusAnaliseEnum.Analisado);
        recuperado.AnaliseResultado.ShouldNotBeNull();
        recuperado.AnaliseResultado.DescricaoAnalise.Valor.ShouldBe("Análise após processamento");
